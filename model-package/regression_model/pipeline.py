...
orders_pipe = Pipeline(
    [
        ...
        ("ConsecutiveDays", pp.ConsecutiveDays(variable=config.config_model.date_var)),
        (
            "ConsecutiveWeeks",
            pp.ConsecutiveWeeks(variable=config.config_model.date_var),
        ),
        (
            "DateAttributes",
            pp.DateAttributes(
        ...
        (
            ...
            pp.CyclicDateAttributes(mapping=config.config_model.cyclic_mapping),
        ),
        ("ShoppingIntensity", pp.ShoppingIntensity()),
        (
            "ProximityHolidays",
            pp.ProximityHolidays(variable=config.config_model.holiday_var),
        ),
        (
            "City2Country",
            pp.City2Country(
        ...
        (
            ...
            pp.TransformOHE(variable=config.config_model.holiday_name_var),
        ),
        (
            "TransformOHE_Warehouse",
            pp.TransformOHE(variable=config.config_model.warehouse_var),
        ),
        ("RowsFilter", pp.RowsFilter(variables=config.config_model.filter_cols)),
        ("ColsDropper", pp.ColsDropper(variables=config.config_model.drop_cols)),
        (
            "Model",
