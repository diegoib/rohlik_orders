    ...
    def fit(self, X: pd.DataFrame, y=None):
        self.mapping_days = {
            ...
            for i, d in enumerate(
                pd.date_range(
                    start=X[self.variable].min(), end=X[self.variable].max(), freq="D"
                )
            )
        }
...
class ConsecutiveWeeks(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X: pd.DataFrame, y=None):
        self.mapping_weeks = {
            ...
            for i, w in enumerate(
                (
                    X[self.variable].dt.year * 100
                    + X[self.variable].dt.isocalendar().week
                ).unique()
            )
        }
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        if (
            ...
            X["cons_week"] = (
                X["date"].dt.year * 100 + X["date"].dt.isocalendar().week
            ).map(self.mapping_weeks)
...
class DateAttributes(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        X["total_holidays_month"] = X.groupby(["year", "month"])["holiday"].transform(
            "sum"
        )
...
class ShoppingIntensity(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        special_days = {
            ...
            "click_frenzy": ((X["day"] == 15) & (X["month"] == 5))
            | ((X["day"] == 15) & (X["month"] == 11)),
        ...
        X["shopping_intensity"] = sum(
            condition.astype(int) for condition in special_days.values()
        )
...
class City2Country(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        X["czech"] = np.where(
            X[self.variable].isin(["Brno_1", "Prague_1", "Prague_2", "Prague_3"]), 1, 0
        )
